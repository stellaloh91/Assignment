...
2) Crosstalk is currently not compatible with visNetwork, an R package for interactive network visualization, Hence, on selecting a location in the network graph, we will have to manually filter a datatable for the location to observe the time each employee visits this location.
...
1) End users need not be adept in programming langauges to be able to utilize the interactivity of the applications and modify the desired criteria for data visualization.
