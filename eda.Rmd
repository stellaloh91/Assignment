...
DT::datatable(location_carid_join, fillContainer = T, extensions='Scroller', filter = "top", options = list(scrollX=TRUE, autoWidth=TRUE, scroller=TRUE, scrollY = '450px'))
```
...
From the above plot, LastName, FirstName, CurrentEmploymentType and CurrentEmploymentTitle have the highest percentage of missing values, followed other variables in the combined GPS and car assignment data dataset. On further investigation, we note that this is because 
...
check_distribution_loyalty_location <- summ_loyalty_data %>%
  ggplot() +
  geom_bar(aes(x=date, y = total_spent, group = 1)) +
