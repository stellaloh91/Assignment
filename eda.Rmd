...
### Missing values
```{r, layout="l-body-outset", fig.width=5.5, fig.height=5, fig.cap="Figure: Loyalty card- Percentage of missing values",tidy.opts=list(width.cutoff=80),tidy=TRUE}
##loyalty card
...
```{r, layout="l-body-outset", fig.width=5.5, fig.height=5, fig.cap="Figure: Credit card- Percentage of missing values",tidy.opts=list(width.cutoff=80),tidy=TRUE}
##credit card
...
```{r, layout="l-body-outset", fig.width=5.5, fig.height=5, fig.cap="Figure: Location data- Percentage of missing values",tidy.opts=list(width.cutoff=80),tidy=TRUE}
##location data
...
From the above plot, LastName, FirstName, CurrentEmploymentType and CurrentEmploymentTitle have the highest percentage of missing values, followed other variables in the combined GPS and car assignment data dataset. On further investigation, we note that this is because there seems to be employees with no specific cars assigned and cars which are not assigned to a specific employee. This results in the joined tables having empty values.

```{r, fig.cap="Table: Employees with no assigned car",tidy.opts=list(width.cutoff=80),tidy=TRUE}
...
```{r, fig.cap="Table: Cars with no assigned employees",tidy.opts=list(width.cutoff=80),tidy=TRUE}
...
```{r, fig.cap="Figure: Distribution of loyalty card spending"}
##loyalty card
...
```{r, fig.cap="Figure: Distribution of credit card spending"}
##credit card
...
```{r, fig.cap="Figure: Log transformed distribution of loyalty card spending", tidy.opts=list(width.cutoff=80),tidy=TRUE}
##loyalty card
...
```{r, fig.cap="Figure: Log transformed distribution of credit card spending", tidy.opts=list(width.cutoff=80),tidy=TRUE}
##credit card
...
The distributions of the log-transformed variables now approximates to the normal distribution. From the above distributions, we can see that the distributions are similar for both loyalty and credit card transactions.
...
```{r, layout="l-screen-inset shaded", fig.height=10, fig.cap="Figure: Loyalty card spending per store", tidy.opts=list(width.cutoff=80),tidy=TRUE}
##loyalty card
...
```{r, layout="l-screen-inset shaded", fig.height=10, fig.cap="Figure: Credit card spending per store", tidy.opts=list(width.cutoff=80),tidy=TRUE}
##credit card
...
```
From the charts above, we noted that there was an additional location for credit card transactions compared to loyalty card transactions- Daily Dealz. There seems to be only one transaction made in this location over the two weeks.

### Outliers

The outliers are also examined using ggplot().

```{r, layout="l-body-outset", fig.width=5.5, fig.height=5, fig.cap="Figure: Distribution of loyalty card spending per store", tidy.opts=list(width.cutoff=80),tidy=TRUE}
##loyalty card
...
```{r, layout="l-body-outset", fig.width=5.5, fig.height=5, fig.cap="Figure: Distribution of credit card spending per store", tidy.opts=list(width.cutoff=80),tidy=TRUE}
...
crosstalk::bscols(gg,DT::datatable(d1), widths = 12)
```

Using the interactive plots above, we are able to select the outliers and this filters the linked datatable for the relevant transaction, giving additional insight such as the card number. We note that there seems to be outliers, the most notable being in Frydos Autosupply n'More where there was a transaction for $10,000 on 01/13/2014 by card ending with 9551. More notably, there was no corresponding transaction on the loyalty card.
