...
2) Based on the timestamp of credit card transactions, we noted that all of the transactions in "Bean There Done That", "Brewed Awakenings", "Coffee Shack" and "Jack's Magical Beans" were made at 12:00pm. It is highly unlikely that all transactions in these locations are transacted at the same time. Hence, the timestamp for these transactions may be incorrect.<br>
...
3) Furthermore, we also noted that there are several transactions in Kronos Mart at odd hours (0300-0359) on 13 January and 19 January. This is highly unusual and warrants further investigation. We will be mindful of using this information for further analysis subsequently.
...
1) Assuming that the car assignment list provided includes all employees, we noted that there are 44 distinct employees. However, we noted that there are 55 distinct credit card numbers and 54 distinct loyalty card numbers. This is unusual as each employee should have been issued a loyalty card and hence we would expect number of distinct credit cards, loyalty cards and employee count to match.<br>
More investigation should be made into this discrepancy. Possible reaons could be that employees could have used more than one credit card with their loyalty card. Another possibility could be that there is a new employee who has not received the loyalty card.<br>
...
```{r, tidy.opts=list(width.cutoff=40),tidy=TRUE, fig.cap="Distinct employee names"}
...
```{r, tidy.opts=list(width.cutoff=35),tidy=TRUE, fig.cap="Distinct loyalty card numbers"}
...
The case scenario does not state which CarIDs are referring to trucks. However, assuming that the 3 digit CarID represents trucks, we only note GPS data for five trucks. There is no evidence as to whether the truck ID is sequential or if each truck driver is assigned to a truck. Given that there are 9 truck drivers and only 5 truck GPS data provided, there is possibility that:
    ...
    b) GPS information for 4 other trucks are missing in the GPS dataset.
...
```{r, tidy.opts=list(width.cutoff=40),tidy=TRUE, fig.cap="CarID with no assigned employee"}
...
![Figure 1: Start new project](Image/NewProject.JPG){width=50%}


3. Add a vector layer. Navigate to Layer > Add Layer > Add Vector Layer

![Figure 2: Add vector layer](Image/AddVectorLayer.JPG){width=70%}


4. Click on the "..." button and navigate to location of Abila.shp file. Click on Add. You should see Figure 4 in your main pane.

![Figure 3: Select SHP file](Image/OpenSHP.JPG)

![Figure 4: Current map](Image/CurrentMap.JPG)


5. To make the map clearer, we will then change the lines from green to black. Right click on Abila under the layers panel > select Properties > select Symbology. click on the arrow dropdown next to Color and select black > Click Apply and Ok. Your screen should resemble Figure 6 below. 

![Figure 5: Select Properties](Image/LayerPanel.JPG){width=50%}

![Figure 6: Change line color](Image/ChangeColor.JPG)
...
![Figure 7: Perform georeferencing](Image/Georeferencer.JPG){width=50%}

![Figure 8: Click on blue symbol](Image/Symbol.JPG){width=50%}


7. To match the corresponding points, click on the identify tool on the top pane (Figure 9). Hover over the suspected corresponding point and click on it. The Identify Results pane appears on the right corner (Figure 10). Observe the results by matching the road name specified on the Identify Results pane to the tourist map.

![Figure 9: Use the identify function to match corresponding points](Image/Identify.JPG)

![Figure 10: Observe results to ensure correct point has been selected](Image/Results.JPG){width=50%}
...
![Figure 11: Select point on the tourist map](Image/SelectPoint.JPG)

![Figure 12: Update map coordinates](Image/MapCoords.JPG){width=50%}

![Figure 13: GCP table](Image/GCP.JPG)
...
![Figure 14: GCP table](Image/TfmSettings.JPG)

![Figure 15: Update settings](Image/UpdateSettings.JPG){width=60%}

![Figure 16: Update Target SRS](Image/TargetSRS.JPG){width=80%}
...
![Figure 17: Start georeferencing](Image/StartGeo.JPG){width=60%}


11. Under the layers pane on the left, drag the image file below the Abila streetmap so that the streetmap can be plotted on top of the tourist map (Figure 18). Doing a check, we observe that the streetmap is well-aligned with the image file. The TIF file created can then be used in RStudio as a raster object.

![Figure 18: Rearrange layers](Image/RearrangeLayers.JPG){width=60%}

![Figure 19: Georeferenced Map](Image/GeoreferencedMap.JPG)


12. After preparing the georeferencing, we will then import the raster layer into RStudio.
...
13. Using st_read() of sf package, import Abila shapefile into R. We will then convert aspatial data to simple feature dataframe.
...
14. Next, join the gps points into movement paths by using the drivers' IDs as unique identifiers.
...
15. Checking the data, we noticed single coordinates pair in the line feature. The following code chunk is to identify and remove the orphan lines.
...
17. Lastly, we then overplot the selected gps path onto the background tourist map.
...
tm_shape(bgmap) +
  ...
  tm_shape(gps_path_selected) +
  tm_lines(col= "red") +
  tm_layout(title="Interactive graph of all cars trajectories from 1200-1259")
```

2) Furthermore as mentioned above, we also noted that there are several transactions in Kronos Mart at odd hours (0300-0359) on 13 January and 19 January. By filtering the map for 3am, we noted that there were no cars near Kronos Mart. Hence, these transactions may be either incorrectly timed or may even be fraudulent. 

```{r, tidy.opts=list(width.cutoff=60),tidy=TRUE, layout="l-body-outset", fig.width=6, fig.height=3, fig.cap="Facet graph of all cars trajectories from 0300-0359"}
##Facet map
gps_path_selected <- gps_path_cleaned %>%
  filter(hour == 3)
...
```{r, layout="l-body-outset", fig.width=12, fig.height=10, fig.cap="Facet network graph by timegroup"}
##Facet network graph
...
![Figure: Select Desafio Golf Course on network graph](Image/Desafio-network.JPG){width=60%}

![Figure: Filter for Desafio Golf Course in datatable](Image/Desafio-dt.JPG){width=60%}

```{r, layout="l-body-outset", fig.width=10, fig.height=9, fig.cap="Interactive network graph"}
##Interactive viz
...
tm_shape(bgmap) +
  ...
  tm_shape(gps_path_selected) +
  tm_lines(col = "red") +
  tm_layout(title="Interactive map of CarID 1 at time 2100-2359")
```
...
tm_shape(bgmap) +
  ...
  tm_shape(gps_path_selected) +
  tm_lines(col= "red") +
  tm_layout(title="Interactive map of CarID 1 on day 13, hour 1700-1759")
```
...
tm_shape(bgmap) +
  ...
  tm_shape(gps_path_selected) +
  tm_lines(col= "red") +
  tm_layout(title="Interactive map of CarID 1 on day 13, timegroup 1300-1359")
```
...
```{r, tidy.opts=list(width.cutoff=60),tidy=TRUE, layout="l-body-outset", fig.width=6, fig.height=7.5, fig.cap="Facet graph of all cars moving at time 2200 - 2359 on day 10"}
##Facet map- by id
gps_path_selected <- gps_path_cleaned %>%
...
```{r, tidy.opts=list(width.cutoff=60),tidy=TRUE, fig.cap="Facet graph of all cars moving at time 0000-0059 on day 11"}
##Facet map- by id
gps_path_selected <- gps_path_cleaned %>%
...
3) **Monitoring of CIO's house at odd hours**
...
- Isia Vann (Perimeter Control, CarID 16) also had a similar trajectory as Loreto Bodrogi. She left her house at 2301 on 01/06/2014 and arrived at the area near Spetson Park at 2309. She then remained in the same area and left only at 0730 on 01/07/2014 and drove directly to Gastech without making any other stops. 
...
tm_shape(bgmap) +
  ...
  tm_shape(gps_path_selected) +
  tm_lines(col= "red") +
  tm_layout(title= "Facet graph of CarID 10 at 1700-1859 (afterwork hours)")
```
...
tm_shape(bgmap) +
  ...
  tm_shape(gps_path_selected) +
  tm_lines(col= "red") +
  tm_layout(title="Interactive graph of CarID 15 on 01/07/2014")
```
...
tm_shape(bgmap) +
  ...
  tm_shape(gps_path_selected) +
  tm_lines(col= "red") +
  tm_layout(title="Interactive graph of CarID 16 on 01/07/2014")
```

4) **Monitoring of COO's house at odd hours**
...
- Loreto Bodrogi (Site Control, CarID 15) visited an area near Taxichorn Park on 01/09/2014 at 0332, departing from his house at 0320. He only departed the area at 0723 and stopped by Jack's Magical Beans before heading directly to Gastech.
...
tm_shape(bgmap) +
  ...
  tm_shape(gps_path_selected) +
  tm_lines(col= "red") +
  tm_layout(title="Interactive graph of CarID 32 at 1700-1800 (afterwork hours)")
```
...
tm_shape(bgmap) +
  ...
  tm_shape(gps_path_selected) +
  tm_lines(col= "red") +
  tm_layout(title="Interactive graph of CarID 15 on 01/09/2014")
```
...
tm_shape(bgmap) +
  ...
  tm_shape(gps_path_selected) +
  tm_lines(col= "red") +
  tm_layout(title="Interactive graph of CarID 24 on 01/09/2014 at 0300-0359")
```

5) **Driving around Abila without making any transactions**
...
tm_shape(bgmap) +
  ...
  tm_shape(gps_path_selected) +
  tm_lines(col= "red") +
  tm_layout(title="Interactive graph of CarID 29 on 01/11/2014, 2100-2359")
```
...
6) **Monitoring of CFO's house at odd hours**
...
```{r, tidy.opts=list(width.cutoff=50),tidy=TRUE, layout="l-body-outset", fig.width=6, fig.height=7.5}
##Interactive map
gps_path_selected <- gps_path_cleaned %>%
...
```{r, tidy.opts=list(width.cutoff=60),tidy=TRUE, fig.cap="Graph of CarID16 at time 2300-2359 on day 10 and 0300-0359 on day 11"}
##Facet map
gps_path_selected <- gps_path_cleaned %>%
...
tm_shape(bgmap) +
  ...
  tm_shape(gps_path_selected) +
  tm_lines(col= "red")  +
  tm_layout(title="Interactive graph of CarID 24 on 01/14/2014")
```
...
tm_shape(bgmap) +
  ...
  tm_shape(gps_path_selected) +
  tm_lines(col= "red") +
  tm_layout(title="Interactive graph of CarID 21 on 01/11/2014")
```
...
7) **Hennie Osvaldo's multiple residences**

Hennie Osavado seems to own two residences- 

i) Area next to Guy's Gyros, 

ii) Area near Frydo's Autosupply N'More. 
...
8) **Potential relationship between Elsa Orilla and Brand Tempestad**
...
9) **Repeated visits to locations not near eateries**
...
i) Area near Frank's Fuels, 

ii) Area west of Bean There Done That, 

iii) Area south of Hallowed Grounds, 

iv) Area south of Katerina's Cafe and 

v) Area south-west of the Arkadious Park. 
...
10) **Suspicious transactions and activities at petrol stations**
...
tm_shape(bgmap) +
  ...
  tm_shape(gps_path_selected) +
  tm_lines(col= "red") +
  tm_layout(title="IInteractive graph of CarID 1 on 01/13/2014 at 1300-1359")
```
...
tm_shape(bgmap) +
  ...
  tm_shape(gps_path_selected) +
  tm_lines(col= "red") +
  tm_layout(title="Interactive graph of CarID 24 on 01/13/2014 at 1200-1359")
```
...
tm_shape(bgmap) +
  ...
  tm_shape(gps_path_selected) +
  tm_lines(col= "red") +
  tm_layout(title="Interactive graph of CarID 29 on 01/11/2014 at 2200-2359")
```
...
```{r, tidy.opts=list(width.cutoff=60),tidy=TRUE, layout="l-body-outset", fig.width=6, fig.height=3, fig.cap="Facet graph by day of the various truck trajectories during 1700-2359"}
##Facet map- by day
gps_path_selected <- gps_path_cleaned %>%
...
12) **Unusual transactions at Kronos Mart**
...
tm_shape(bgmap) +
  ...
  tm_shape(gps_path_selected) +
  tm_lines(col = "red") +
  tm_layout(title="Interactive graph of CarID 32 on 01/11/2014 and 01/12/2014")
```
...
tm_shape(bgmap) +
  ...
  tm_shape(gps_path_selected) +
  tm_lines(col = "red") +
  tm_layout(title="Interactive graph of CarID 23 on 01/18/2014 and 01/19/2014")
```

```{r, tidy.opts=list(width.cutoff=50),tidy=TRUE, fig.cap="Facet graph of CarID 1"}
##Interactive map
gps_path_selected <- gps_path_cleaned %>%
...
tm_shape(bgmap) +
  ...
  tm_shape(gps_path_selected) +
  tm_lines(col = "red") +
  tm_layout(title="Interactive graph of CarID 1 on 01/18/2014 and 01/19/2014")
```
...
tm_shape(bgmap) +
  ...
  tm_shape(gps_path_selected) +
  tm_lines(col = "red") +
  tm_layout(title="Interactive graph of CarID 10 on 01/18/2014 and 01/19/2014")
```

13) **Visits to Kronos Capitol**
...
tm_shape(bgmap) +
  ...
  tm_shape(gps_path_selected) +
  tm_lines(col= "red") +
  tm_layout(title="Interactive graph of CarID 35 on 01/11/2014 at 1300- 1959")
```
...
tm_shape(bgmap) +
  ...
  tm_shape(gps_path_selected) +
  tm_lines(col= "red") +
  tm_layout(title="Interactive graph of of various car trajectories on 01/18/2014")
```
...
tm_shape(bgmap) +
  ...
  tm_shape(gps_path_selected) +
  tm_lines(col= "red") +
  tm_layout(title="Interactive graph of CarID 35 trajectory across 2 week period")
```
...
```{r, tidy.opts=list(width.cutoff=60),tidy=TRUE, layout="l-body-outset", fig.width=6, fig.height=1.5, fig.cap="Facet graph of various executives' car trajectories on 01/12/2014, 1200-1459"}
##Facet map
gps_path_selected <- gps_path_cleaned %>%
...
```{r, tidy.opts=list(width.cutoff=60),tidy=TRUE, layout="l-body-outset", fig.width=6, fig.height=3, fig.cap="Facet graph of various executives' car trajectories on 01/19/2014, 1200-1359"}
##Facet map
gps_path_selected <- gps_path_cleaned %>%
...
```{r, tidy.opts=list(width.cutoff=60),tidy=TRUE, layout="l-body-outset", fig.width=6, fig.height=1.5, fig.cap="Facet graph of CarID 31"}
##Facet map
gps_path_selected <- gps_path_cleaned %>%
  filter(id == 31)
